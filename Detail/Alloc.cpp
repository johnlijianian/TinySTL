...
namespace TinySTL{
    ...
    void *alloc::refill(size_t bytes){
        size_t nobjs = ENObjs::NOBJS; //每次增加的节点数量
        //从内存池里取
        ...
        if (nobjs == 1){//取出的空间只够一个对象使用
            ...
            *my_free_list = next_obj = (obj *)(chunk + bytes);
            ...
            for (int i = 1;; ++i){
                ...
                next_obj = (obj *)((char *)next_obj + bytes); //连续空间下
                if (nobjs - 1 == i){
                    current_obj->next = 0;
                    break;
                }
    ...
    char *alloc::chunk_alloc(size_t bytes, size_t& nobjs){
        ...
        if (bytes_left >= total_bytes){//内存池剩余空间完全满足需要
            ...
            if (bytes_left > 0){ //把内存池当前剩下的一些小残余零头利用一下。
                obj **my_free_list = free_list + FREELIST_INDEX(bytes_left);
                ((obj *)start_free)->next = *my_free_list;
                *my_free_list = (obj *)start_free;
            }
            start_free = (char *)malloc(bytes_to_get); // 进行分配
            if (!start_free) { // 如果失败，找到free_list中尚未使用且足够大的区域
